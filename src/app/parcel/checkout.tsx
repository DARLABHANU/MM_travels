...
export default function ParcelCheckout() {
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <Text style={styles.title}>Parcel Service Checkout</Text>
            <Text>Intent: {intent?.serviceId}</Text>
            <Text>Sender: {locations.sender?.address?.name || 'Missing'}</Text>
            <Text>Receiver: {locations.receiver?.address?.name || 'Missing'}</Text>
        </SafeAreaView>
    );
}
